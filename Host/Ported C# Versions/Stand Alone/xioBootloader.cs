...
namespace Bootloader
{
    class xioBootloader
    {
        ...
        #region Constants
        const int PM_SIZE = 1536;
        const int EE_SIZE = 128;
        const int CM_SIZE = 8;
        const int BUFFER_SIZE = 4096;
        const int PM30F_ROW_SIZE = 32;
        const int PM33F_ROW_SIZE = 64 * 8;
        const int EE30F_ROW_SIZE = 16;

        const byte COMMAND_NACK = 0x00;
        const byte COMMAND_ACK = 0x01;
        const byte COMMAND_READ_PM = 0x02;
        const byte COMMAND_WRITE_PM = 0x03;
        const byte COMMAND_READ_EE = 0x04;
        const byte COMMAND_WRITE_EE = 0x05;
        const byte COMMAND_READ_CM = 0x06;
        const byte COMMAND_WRITE_CM = 0x07;
        const byte COMMAND_RESET = 0x08;
        const byte COMMAND_READ_ID = 0x09;
        #endregion
        //Bootloader required types
        public enum eFamily { dsPIC30F, dsPIC33F, PIC24H, PIC24F };
        ...
        class memRow
        {
            ...
            public enum eType { Program = 0, EEprom, Config };
        ...
        /// <summary>
        /// Constructor - Creates an instance of the bootloader
        /// </summary>
        /// <param name="_ComPort">Name of the port to be used, ie COM1</param>
        /// <param name="_FileWithPath">File name with full path of a HEX file</param>
        /// <param name="ProgressFormLocation">The location of the progress form</param>
        public xioBootloader(string _ComPort, string _FileWithPath, Point ProgressFormLocation)
        {
            Port = new SerialPort(_ComPort, 115200, Parity.None, 8, StopBits.One);
            Port.ReadTimeout = 500; //long, so obvious if going wrong
            Port.WriteTimeout = 500;

            ProgressForm = new Form();

            ProgressForm.Text = "Uploading New Firmware";
            ProgressForm.Width = 285;
            ProgressForm.Height = 84;
            ProgressForm.Location = ProgressFormLocation;

            Label aLabel = new Label();
            aLabel.Top = 30; aLabel.Left = 9; aLabel.AutoSize = false; aLabel.Width = 253; aLabel.TextAlign = ContentAlignment.TopLeft;
            aLabel.Text = "Preparing data...";
            ProgressForm.Controls.Add(aLabel);

            ProgressBar aBar = new ProgressBar();
            aBar.Top = 12; aBar.Left = 12; aBar.Width = 253; aBar.Height = 15; aBar.Value = 0; aBar.Minimum = 0; aBar.Maximum = 100;
            ProgressForm.Controls.Add(aBar);
        ...
        //other functions
        void UpdateForm(string _message, int _progress)
        {
            ((Label)ProgressForm.Controls[0]).Text = _message;
            ((ProgressBar)ProgressForm.Controls[1]).Value = _progress;
        }

        /// <summary>
        /// This even occurs when the board was programmed successfully
        /// </summary>
        /// <param name="sender"></param>
        public delegate void OnBootloadFinished(object sender);
        public event OnBootloadFinished BootloadFinished;
        protected virtual void onBootloadFinished()
        {
            if (BootloadFinished != null)
                BootloadFinished(this);
        }
